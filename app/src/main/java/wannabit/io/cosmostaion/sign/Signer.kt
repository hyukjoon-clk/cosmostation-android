...
import com.gno.vm.VmProto.MemFile
import com.gno.vm.VmProto.MemPackage
import com.gno.vm.VmProto.MsgAddPackage
import com.gno.vm.VmProto.MsgCall
import com.gno.vm.VmProto.MsgRun
...
object Signer {
    ...
    fun compoundingMsg(
        ...
        rewards.forEach { reward ->
            ...
            val rewardCoin =
                reward?.rewardList?.firstOrNull { it.denom == selectedChain.getStakeAssetDenom() }
    ...
    fun mintPhotonMsg(msgMintPhoton: MsgMintPhoton?): MutableList<Any> {
        val msgAnys: MutableList<Any> = mutableListOf()
        msgAnys.add(
            Any.newBuilder().setTypeUrl("/atomone.photon.v1.MsgMintPhoton")
                .setValue(msgMintPhoton?.toByteString()).build()
        )
        return msgAnys
    }
    ...
    fun grpcByteSignature(selectedChain: BaseChain?, toSignByte: ByteArray?): ByteArray {
    ...
    private fun encodeULEB128(value: Int): ByteArray {
        ...
        do {
            ...
            v = v ushr 7
            if (v != 0) byte = (byte.toInt() or 0x80).toByte()
            result.add(byte)
        } while (v != 0)
